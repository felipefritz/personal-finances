...
from app.routers.bank_connections import auto_sync_connections_once


logger = logging.getLogger(__name__)


async def _bank_auto_sync_loop() -> None:
    """Run bank-scraper syncs on a recurring background schedule.

    Calls ``auto_sync_connections_once`` in a thread pool to avoid blocking the
    event loop, then sleeps for the configured interval.  Errors are caught and
    logged so that one failed cycle does not kill the loop.
    """
    interval_seconds = max(600, int(settings.BANK_AUTO_SYNC_INTERVAL_SECONDS or 21600))
    while True:
        try:
            summary = await asyncio.to_thread(auto_sync_connections_once)
            logger.info("Bank auto-sync cycle finished: %s", summary)
        except Exception as exc:
            logger.exception("Bank auto-sync cycle failed: %s", exc)
...
async def lifespan(app: FastAPI):
    ...
    Startup sequence:
        ...
        3. Launch the bank background sync loop when ``BANK_AUTO_SYNC_ENABLED`` is set.
    ...
    if settings.BANK_AUTO_SYNC_ENABLED:
        auto_sync_task = asyncio.create_task(_bank_auto_sync_loop())
...
app = FastAPI(
    ...
    description="API para gestion de finanzas personales, importaciones, proyecciones y sincronizacion bancaria local.",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan,
)
...
app.include_router(money_allocations_router, prefix=API_PREFIX)
